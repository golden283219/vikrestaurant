...
<script>
	 ...
	 * @var object
	 */
	if (typeof WEEKRES_CHARTS === 'undefined') {
		var WEEKRES_CHARTS = {};
	}
	...
	WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = function(widget, json, config) {
		...
		if (!WEEKRES_CHARTS.hasOwnProperty(id)) {
			// prepare chart data
			var chartData = {
			...
			WEEKRES_CHARTS[id] = new Chart(ctx, {
				type:    'bar',
				data:    chartData,
				options: options,
			});
		...
		// otherwise update labels and values
			...
			// update chart data
			WEEKRES_CHARTS[id].data.labels = Object.keys(data);
			WEEKRES_CHARTS[id].data.datasets[0].data = Object.values(data);

			// update chart colors
			WEEKRES_CHARTS[id].data.datasets[0].backgroundColor = '#' + config.color + '99';
			WEEKRES_CHARTS[id].data.datasets[0].borderColor     = '#' + config.color;

			// refresh chart
			WEEKRES_CHARTS[id].update();
		}
	}
