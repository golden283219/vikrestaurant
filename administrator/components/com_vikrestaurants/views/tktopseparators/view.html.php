...
class VikRestaurantsViewtktopseparators extends JViewVRE
{
	...
	function display($tpl = null)
	{
		...
		$ordering = OrderingManager::getColumnToOrder('tktopseparators', 's.ordering', 1);

		// set the toolbar
		$this->addToolBar();

		$filters = array();
		$filters['keysearch'] = $app->getUserStateFromRequest('vre.tktopseparators.keysearch', 'keysearch', '', 'string');
		...
		{
			$q->where($dbo->qn('s.title') . ' LIKE ' . $dbo->q("%{$filters['keysearch']}%"));
		}
	...
	private function addToolBar()
	{
		...
		JToolbarHelper::title(JText::_('VRMAINTITLEVIEWTKTOPPINGSEPS'), 'vikrestaurants');
		
		if (JFactory::getUser()->authorise('core.create', 'com_vikrestaurants'))
		{
			JToolbarHelper::addNew('tktopseparator.add', JText::_('VRNEW'));
			JToolbarHelper::divider();
		}

		if (JFactory::getUser()->authorise('core.edit', 'com_vikrestaurants'))
		{
			JToolbarHelper::editList('tktopseparator.edit', JText::_('VREDIT'));
			JToolbarHelper::spacer();
		}

		if (JFactory::getUser()->authorise('core.delete', 'com_vikrestaurants'))
		{
			JToolbarHelper::deleteList(VikRestaurants::getConfirmSystemMessage(), 'tktopseparator.delete', JText::_('VRDELETE'));
		}
	}
}
