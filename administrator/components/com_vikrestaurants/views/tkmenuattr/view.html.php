...
class VikRestaurantsViewtkmenuattr extends JViewVRE
{
	...
	function display($tpl = null)
	{
		...
		$filters = array();
		$filters['keysearch'] = $app->getUserStateFromRequest('vre.tkmenuattr.keysearch', 'keysearch', '', 'string');
		$filters['status']    = $app->getUserStateFromRequest('vre.tkmenuattr.status', 'status', '', 'string');
		...
		{
			$attribute['languages'] = array($def_lang);

			$q = $dbo->getQuery(true)
				->select($dbo->qn('tag'))
				->from($dbo->qn('#__vikrestaurants_lang_takeaway_menus_attribute'))
				->where($dbo->qn('id_attribute') . ' = ' . $attribute['id']);
			...
			if ($dbo->getNumRows())
			{
				...
				$attribute['languages'] = array_unique(array_merge($attribute['languages'], $dbo->loadColumn()));
			}
		}

		$new_type = OrderingManager::getSwitchColumnType('tkmenuattr', $ordering['column'], $ordering['type'], array(1, 2));
	...
	private function addToolBar()
	{
		...
		JToolbarHelper::title(JText::_('VRMAINTITLEVIEWTKMENUATTRS'), 'vikrestaurants');
		
		if (JFactory::getUser()->authorise('core.create', 'com_vikrestaurants'))
		{
			JToolbarHelper::addNew('tkattribute.add', JText::_('VRNEW'));
			JToolbarHelper::divider();
		}

		if (JFactory::getUser()->authorise('core.edit', 'com_vikrestaurants'))
		{
			JToolbarHelper::editList('tkattribute.edit', JText::_('VREDIT'));
			JToolbarHelper::spacer();
		}

		if (JFactory::getUser()->authorise('core.delete', 'com_vikrestaurants'))
		{
			JToolbarHelper::deleteList(VikRestaurants::getConfirmSystemMessage(), 'tkattribute.delete', JText::_('VRDELETE'));
		}
	}
