...
$links = array(
	...
	OrderingManager::getLinkColumnOrder('tkmenuattr', '<i class="fas fa-sort"></i>', 'ordering', $ordering['ordering'], 1, $filters, 'vrheadcolactive'.(($ordering['ordering'] == 2) ? 1 : 2)),
);
...
{
	$saveOrderingUrl = 'index.php?option=com_vikrestaurants&task=tkattribute.saveOrderAjax&tmpl=component';
	JHtml::_('vrehtml.scripts.sortablelist', 'menuattrList', 'adminForm', $orderDir, $saveOrderingUrl);
}
...
if (count($rows) == 0)
{
	echo $vik->alert(JText::_('VRNOTKMENUATTR'));
}
else
{
	?>
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>" id="menuattrList">
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				<th class="<?php echo $vik->getAdminThClass(); ?>" width="5%" style="text-align: center;"><?php echo JText::_('VRMANAGETKMENUATTR3'); ?></th>
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="5%" style="text-align: center;"><?php echo JText::_('VRMANAGETKMENUATTR4'); ?></th>
		...
		for ($i = 0; $i < count($rows); $i++)
		{
			...
			<tr class="row<?php echo $kk; ?>">
				...
				<td>
					...
					{
						?>
						<a href="index.php?option=com_vikrestaurants&amp;task=tkattribute.edit&amp;cid[]=<?php echo $row['id']; ?>"><?php echo $row['name']; ?></a>
						<?php
					}
					else
					{
				...
				<td style="text-align: center;">
					...
					{
						?>
						<a href="index.php?option=com_vikrestaurants&amp;task=tkattribute.<?php echo ($row['published'] ? 'un' : ''); ?>publish&amp;cid[]=<?php echo $row['id']; ?>">
				...
				<td style="text-align: center;" class="hidden-phone">
					...
					{
						?>
						<img src="<?php echo VREMEDIA_SMALL_URI . $row['icon']; ?>" />
						<?php
					}
					?>
				</td>
				...
				if ($multi_lang)
				{
					?>
					<td style="text-align: center;">
						<a href="index.php?option=com_vikrestaurants&amp;view=langtkattributes&amp;id_attribute=<?php echo $row['id']; ?>">
							<?php
...
?>
	...
	<input type="hidden" name="view" value="tkmenuattr" />
