...
<div class="row-fluid">
	...
	<?php
	for ($j = 0; $j < count($this->customFields); $j++)
	{
		?>
		<div class="span6">
			<?php
			echo $vik->openFieldset(JText::_('VRCUSTOMFGROUPOPTION' . ($j + 1)), 'form-horizontal');
				
				foreach ($this->customFields[$j] as $i => $cf)
				{
					// fetch fields to use
					$fields = $j == 0 ? $this->customer->fields : $this->customer->tkfields;
					...
					{
						echo $vik->calendar($_val, 'vrcf' . $cf->id, 'vrcf' . $cf->id . 'date');
					}
