...
class VikRestaurantsViewtkreservations extends JViewVRE
{
	...
	function display($tpl = null)
	{
		...
		$filters = array();
		$filters['datefilter']  = $app->getUserStateFromRequest('vre.tkreservations.datefilter', 'datefilter', '', 'string');
		$filters['shift']       = $app->getUserStateFromRequest('vre.tkreservations.shift', 'shift', '', 'string');
		$filters['keysearch']   = $app->getUserStateFromRequest('vre.tkreservations.keysearch', 'keysearch', '', 'string');
		$filters['ordstatus']   = $app->getUserStateFromRequest('vre.tkreservations.ordstatus', 'ordstatus', '', 'string');
		$filters['service']     = $app->getUserStateFromRequest('vre.tkreservations.service', 'service', '', 'string');
		$filters['id_operator'] = $app->getUserStateFromRequest('vre.tkreservations.id_operator', 'id_operator', 0, 'uint');

		// this filters comes only from offline credit card payment
		$filters['ids']	= $app->getUserStateFromRequest('vre.tkreservations.ids', 'ids', array(), 'uint');
		...
		if ($filters['datefilter'])
		{
			...
			if ($filters['shift'])
			{
				...
				$q->where('DATE_FORMAT(FROM_UNIXTIME(' . $dbo->qn('r.checkin_ts') . '), \'%H\') BETWEEN ' . $time->fromhour . ' AND ' . $time->tohour);
			}
		}
		...
		if (strlen($filters['service']))
		{
			$q->where($dbo->qn('r.delivery_service') . ' = ' . (int) $filters['service']);
		}
		...
		$custom_fields = VRCustomFields::getList(1, $mask);
	...
	private function addToolBar()
	{
		...
		JToolbarHelper::title(JText::_('VRMAINTITLEVIEWTKRES'), 'vikrestaurants');

		$user = JFactory::getUser();

		if ($user->authorise('core.create', 'com_vikrestaurants'))
		{
			JToolbarHelper::addNew('tkreservation.add', JText::_('VRNEW'));
			JToolbarHelper::divider();
		}

		if ($user->authorise('core.edit', 'com_vikrestaurants'))
		{	
			JToolbarHelper::editList('tkreservation.edit', JText::_('VREDIT'));
			JToolbarHelper::spacer();
			
			JToolbarHelper::custom('tkreservation.editbill', 'tag-2', 'tag-2', JText::_('VRBILL'), true, false);
			JToolbarHelper::spacer();
		}
		...
		{
			JToolbarHelper::custom('tkreservation.sendsms', 'comment', 'comment', JText::_('VRSENDSMS'), true);
			JToolbarHelper::divider();
		}

		if ($user->authorise('core.delete', 'com_vikrestaurants'))
		{
			JToolbarHelper::deleteList(VikRestaurants::getConfirmSystemMessage(), 'tkreservation.delete', JText::_('VRDELETE'));    
		}		
	}
