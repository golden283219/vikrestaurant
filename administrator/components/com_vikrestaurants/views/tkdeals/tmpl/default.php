...
foreach (array('id', 'name', 'type', 'published', 'ordering') as $c)
{
	if (empty($ordering[$c]))
	{
		$ordering[$c] = 0;
...
$links = array(
	...
	OrderingManager::getLinkColumnOrder('tkdeals', '<i class="fas fa-sort"></i>', 'ordering', $ordering['ordering'], 1, $filters, 'vrheadcolactive'.(($ordering['ordering'] == 2) ? 1 : 2)),
);
...
{
	$saveOrderingUrl = 'index.php?option=com_vikrestaurants&task=tkdeal.saveOrderAjax&tmpl=component';
	JHtml::_('vrehtml.scripts.sortablelist', 'dealsList', 'adminForm', $orderDir, $saveOrderingUrl);
}
...
<form action="index.php?option=com_vikrestaurants" method="post" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar hidden-phone" id="vr-search-tools" style="height: 32px;<?php echo ($has_filters ? '' : 'display: none;'); ?>">
		...
		$deals = DealsHandler::getSupportedDeals();

		foreach ($deals as $deal)
		{
			$options[] = JHtml::_('select.option', $deal->getID(), $deal->getName());
		}
		?>
		<div class="btn-group pull-left">
...
if (count($rows) == 0)
{
	echo $vik->alert(JText::_('VRNOTKDEAL'));
}
else
{
	?>
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>" id="dealsList">
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="10%" style="text-align: left;"><?php echo $links[2]; ?></th>
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="10%" style="text-align: center;"><?php echo JText::_('VRMANAGETKDEAL4'); ?></th>
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="10%" style="text-align: center;"><?php echo JText::_('VRMANAGETKDEAL5'); ?></th>
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="7%" style="text-align: center;"><?php echo JText::_('VRMANAGETKDEAL6'); ?></th>
				<th class="<?php echo $vik->getAdminThClass(); ?>" width="7%" style="text-align: center;"><?php echo $links[3]; ?></th>
				
				<?php
				if ($multi_lang)
				{
					?>
					<th class="<?php echo $vik->getAdminThClass(); ?>" width="7%" style="text-align: center;"><?php echo JText::_('VRMANAGEMENU33');?></th>
					<?php
				}
				?>
		...
		for ($i = 0; $i < count($rows); $i++)
		{
			...
			// fetch deal name
			$deal_name = isset($deals[$row['type']]) ? $deals[$row['type']]->getName() : '--';
			?>
			<tr class="row<?php echo $kk; ?>">
				...
				<td>
					...
					{
						?>
						<a href="index.php?option=com_vikrestaurants&amp;task=tkdeal.edit&amp;cid[]=<?php echo $row['id']; ?>"><?php echo $row['name']; ?></a>
						<?php
					}
					else
					{
					...
					<div class="mobile-only">
						<span class="badge"><?php echo $deal_name; ?></span>
					</div>
				</td>
				...
				<td style="text-align: center;" class="hidden-phone">
					<?php echo ($row['start_ts'] > 0 ? date($date_format, $row['start_ts']) : '/'); ?>
				</td>

				<td style="text-align: center;" class="hidden-phone">
					<?php echo ($row['end_ts'] > 0 ? date($date_format, $row['end_ts']) : '/'); ?>
				</td>

				<td style="text-align: center;" class="hidden-phone">
					<?php echo ($row['max_quantity'] > 0 ? $row['max_quantity'] : '/'); ?>
				</td>
				...
				<td style="text-align: center;">
					...
					{
						?>
						<a href="index.php?option=com_vikrestaurants&amp;task=tkdeal.<?php echo ($row['published'] ? 'un' : ''); ?>publish&amp;cid[]=<?php echo $row['id']; ?>">
				...
				if ($multi_lang)
				{
					?>
					<td style="text-align: center;">
						<a href="index.php?option=com_vikrestaurants&amp;view=langtkdeals&amp;id_deal=<?php echo $row['id']; ?>">
							<?php
...
?>
	...
	<input type="hidden" name="view" value="tkdeals" />
