...
class VikRestaurantsControllerCoupon extends VREControllerAdmin
{
	...
	public function save()
	{
		...
		$args['datestart']  = $input->get('dstart', '', 'string');
		$args['dateend']    = $input->get('dend', '', 'string');
		$args['minvalue']   = $input->get('minvalue', 0.0, 'float');
		$args['usages']     = $input->get('usages', 0, 'uint');
		$args['maxusages']  = $input->get('maxusages', 0, 'uint');
		...
		$this->setRedirect('index.php?option=com_vikrestaurants&task=coupon.edit&cid[]=' . $coupon->id);
	...
	public function delete()
	{
		...
		JTableVRE::getInstance('coupon', 'VRETable')->delete($cid);
