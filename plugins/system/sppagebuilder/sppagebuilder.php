...
class  plgSystemSppagebuilder extends CMSPlugin
{
	...
	function onAfterRender()
	{
		...
		if ($app->isClient('administrator'))
		{
			...
			{
				$dashboardHTML = '<a href="' . $frontendEditorLink . '" target="_blank" class="sp-pagebuilder-button builder-button-primary">Edit with SP Page Builder</a>';
			}
	...
	private static function getPageContent($extension = 'com_content', $extension_view = 'article', $view_id = 0)
	{
		...
		$query->select($db->quoteName(array('id', 'text', 'active')));
