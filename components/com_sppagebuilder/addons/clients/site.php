...
class SppagebuilderAddonClients extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$carousel_item_number_xxl = (isset($settings->carousel_item_number_xxl) && $settings->carousel_item_number_xxl) ? $settings->carousel_item_number_xxl : 6;
		$carousel_item_number_xl = (isset($settings->carousel_item_number_xl) && $settings->carousel_item_number_xl) ? $settings->carousel_item_number_xl : 5;
		$carousel_item_number_lg = (isset($settings->carousel_item_number_lg) && $settings->carousel_item_number_lg) ? $settings->carousel_item_number_lg : 4;
		...
		$output   = '';
		$output  = '<div class="sppb-addon sppb-addon-clients' . '' . $class . '' . ($create_carousel ? ' sppb-carousel-extended' : '') . '"  data-arrow="' . $carousel_arrow . '" data-left-arrow="fa-angle-left" data-right-arrow="fa-angle-right" data-dots="' . $carousel_bullet . '" data-autoplay="' . $carousel_autoplay . '" data-speed="' . $carousel_speed . '" data-interval="' . $carousel_interval . '" data-margin="' . $carousel_margin . '" data-item-number-xxl="' . $carousel_item_number_xxl . '" data-item-number-xl="' . $carousel_item_number_xl . '" data-item-number-lg="' . $carousel_item_number_lg . '" data-item-number-md="' . $carousel_item_number_md . '" data-item-number-sm="' . $carousel_item_number_sm . '" data-item-number-xs="' . $carousel_item_number_xs . '">';
			...
			foreach ($settings->sp_clients_item as $key => $value) {
				...
				if ($client_img_src) {
					...
					if (strpos($client_img_src, "http://") !== false || strpos($client_img_src, "https://") !== false) {
						...
						$client_img_src = Uri::base(true) . '/' . $client_img_src;
					}

					$placeholder = $client_img_src == '' ? false : $this->get_image_placeholder($client_img_src);

					$columns = str_replace('sppb-col-sm-', 'sppb-col-', $columns);
	...
	public static function getTemplate()
	{
		...
		let carousel_item_number_xxl = 1;
		let carousel_item_number_xl = 1;
		let carousel_item_number_lg = 1;
		let carousel_item_number_md = 1;
        let carousel_item_number_sm = 1;
        let carousel_item_number_xs = 1;
		
        if(_.isObject(data.carousel_item_number)){
			const {carousel_item_number} = data;
			const {xll, xl, lg, md, sm, xs} = carousel_item_number;
			
			carousel_item_number_xxl = parseInt(xll) ? xll: 1;
			carousel_item_number_xl = parseInt(xl) ? xl : 1;
			carousel_item_number_lg = parseInt(lg) ? lg : 1;
            carousel_item_number_md = parseInt(md) ? md : 1;
            carousel_item_number_sm = parseInt(sm) ? sm : 1;
            carousel_item_number_xs = parseInt(xs) ? xs : 1;
		}
		let column_class  = data.count.replace("sppb-col-sm-", "sppb-col-");
		#>
		<div class="sppb-addon sppb-addon-clients {{ data.class }} {{(data.create_carousel ? \' sppb-carousel-extended\' : \'\')}}" data-left-arrow="fa-angle-left" data-right-arrow="fa-angle-right" data-arrow="{{data.carousel_arrow}}" data-dots="{{data.carousel_bullet}}" data-autoplay="{{data.carousel_autoplay}}" data-speed="{{data.carousel_speed || 2000}}" data-interval="{{data.carousel_interval ||3500}}" data-margin="{{data.carousel_margin}}" data-item-number-xxl="{{carousel_item_number_xxl}}" data-item-number-xl="{{carousel_item_number_xl}}" data-item-number-lg="{{carousel_item_number_lg}}" data-item-number-md="{{carousel_item_number_md}}" data-item-number-sm="{{carousel_item_number_sm}}" data-item-number-xs="{{carousel_item_number_xs}}">
