...
class SppagebuilderAddonImage_layouts extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		else
		{
			...
			if ($image_preset_thumb === 'card' || $image_preset_thumb === 'overlap' || $image_preset_thumb === 'collage')
			{
				...
				$output .= '<div class="sppb-col-' . ($image_container_column ? $image_container_column : 6) . '' . $order_class . '">';
			}
			...
			{
				$output .= '</div>';
				$output .= '<div class="sppb-col-' . ($image_container_column ? ($image_container_column == 12 ? 12 : 12 - $image_container_column) : 6) . '' . $cont_order_class . '' . ($image_preset_thumb === 'collage' ? ' collage-content-vertical-' . $content_vertical_align : '') . '">';
			}
	...
	{
		return array(Uri::base() . '/components/com_sppagebuilder/assets/js/jquery.magnific-popup.min.js');
	}
	...
	{
		return array(Uri::base() . '/components/com_sppagebuilder/assets/css/magnific-popup.css');
	}
	...
	public static function getTemplate()
	{
		...
		$output .= $lodash->typography('#btn-{{ data.id }}.sppb-btn-{{ data.type }}', 'data.button_typography', $btnTypographyFallbacks);
		...
		<# } else { #>
			...
			<# if(image_preset_thumb === "card" || image_preset_thumb === "overlap" || image_preset_thumb === "collage"){ #>
				<div class="sppb-row">
				<div class="sppb-col-{{(data.image_container_column ? data.image_container_column : 6)}}{{order_class}}">
			<# } #>
			...
			#>
				...
				<div class="sppb-col-{{(data.image_container_column ? (data.image_container_column == 12 ? 12 : 12-data.image_container_column) : 6)}}{{cont_order_class}}{{collage_content_vertical}}">
			<# } #>
			<# 
