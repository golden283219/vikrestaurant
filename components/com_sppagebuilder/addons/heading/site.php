...
class SppagebuilderAddonHeading extends SppagebuilderAddons
{
	...
	public function css()
	{
		...
		$settings->title_color = CSSHelper::parseColor($settings, 'title_color');
		...
		$titleProps = [
			...
			'title_color'    	   => $colorType != "solid" ? '-webkit-background-clip: text; -webkit-text-fill-color: transparent; background-image' : 'color',
			'title_text_shadow'	  => 'text-shadow'
		];

		$units = ['title_margin' => false, 'title_padding' => false, 'title_color' => false, 'title_text_shadow' => false];
