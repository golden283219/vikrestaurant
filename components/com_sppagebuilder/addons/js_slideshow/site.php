	...
	public function render()
	{
		...
		if (isset($settings->slideshow_items) && is_array($settings->slideshow_items))
		{
			...
			foreach ($settings->slideshow_items as $item_key => $item_value)
			{
				...
				$image_column_width = (isset($item_value->image_column_width->xl) && $item_value->image_column_width->xl) ? $item_value->image_column_width->xl : 6;
				$image_column_width_lg = (isset($item_value->image_column_width->lg) && $item_value->image_column_width->lg) ? $item_value->image_column_width->lg : 6;
				$image_column_width_md = (isset($item_value->image_column_width->md) && $item_value->image_column_width->md) ? $item_value->image_column_width->md : 6;
				...
				else
				{
					...
					if (!$image_column_reverse)
					{
						...
						$output .= '<div class="sppb-col-xs-' . $image_column_width_xs . ' sppb-col-sm-' . $image_column_width_sm . ' sppb-col-md-' . $image_column_width_md . ' sppb-col-lg-' . $image_column_width_lg . ' sppb-col-' . $image_column_width . ' image-align-' . $image_content_alignment . '">';
			...
			foreach ($settings->slideshow_items as $item_key => $item_value)
			{
				...
				if (isset($item_value->slideshow_inner_items) && is_array($item_value->slideshow_inner_items))
				{
					foreach ($item_value->slideshow_inner_items as $inner_item_key => $inner_value)
					{
						...
						//Content css
						if ($content_type !== 'btn_content') {
							$css .= $cssHelper->generateStyle(".sp-slider {$uniqid} {$inner_uniqid}", $inner_value, $contentStyleProps, ['content_color' => false, 'content_margin' => false,'content_padding' => false, 'content_border' => false, 'content_border_color' => false]);
						...
						$css .= $cssHelper->generateStyle(".sp-slider {$uniqid} {$inner_uniqid}.sppb-sp-slider-button .sp-slider-btn-text", $inner_value, array_merge($buttonNormalProps, ["content_margin" => "margin", "content_color" => "color"],["content_padding" => "padding","content_border" => "border-style: solid;border-width", "content_border_color" => "border-color", "content_border_radius" => "border-radius"] ), false, ["content_margin" => "spacing", "content_padding" => "spacing"]);
	...
	public static function getTemplate()
	{
		...
		$output .= '<#
			if (_.isEmpty(inner_value.image_content_height)) {
				inner_value.image_content_height = {xxl: 385, xl: 385, lg: 385, md: "", sm: "", xs: ""};
			}
		...
		$output .= '
			...
			#>
					...
					<# }
						...
						var image_column_width = (_.isObject(item_value.image_column_width) && item_value.image_column_width.xl) ? item_value.image_column_width.xl : 6;
						var image_column_width_lg = (_.isObject(item_value.image_column_width) && item_value.image_column_width.lg) ? item_value.image_column_width.lg : 6;
						var image_column_width_md = (_.isObject(item_value.image_column_width) && item_value.image_column_width.md) ? item_value.image_column_width.md : 6;
					...
					#>
							...
							<# } else { #>
								<div class="sppb-row">
									<# if(!image_column_reverse){ #>
										<div class="sppb-col-xs-{{12-image_column_width_xs}} sppb-col-sm-{{12-image_column_width_sm}} sppb-col-md-{{12-image_column_width_md}} sppb-col-lg-{{12-image_column_width_lg}} sppb-col-{{12-image_column_width}}">
											<div class="sp-slider-content-align-{{content_alignment}}">
												<#
												_.each(item_value.slideshow_inner_items, function(inner_value, inner_item_key){
													let last_field_inner_key = `slideshow_inner_items-${inner_item_key}`;
										...
										<div class="sppb-col-xs-{{image_column_width_xs}} sppb-col-sm-{{image_column_width_sm}} sppb-col-md-{{image_column_width_md}} sppb-col-lg-{{image_column_width_lg}} sppb-col-{{image_column_width}} image-align-{{image_content_alignment}}">
