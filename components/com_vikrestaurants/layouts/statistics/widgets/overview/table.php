									...
									if (!$colspan || $table->multi_res)
									{
										...
										if ($info === null)
										{
											...
											else if (strtotime('+' . $filters['intervals'] . ' minutes', $ts) > $now)
											{
												...
												$link_href = 'index.php?option=com_vikrestaurants&task=opreservation.add&date=' . $search->get('date') . '&hourmin=' . $hourmin->value . '&people=' . $table->min_capacity . '&idt=' . $table->id;
												$link_href = JRoute::_($link_href . '&from=opdashboard' . ($itemid ? '&Itemid=' . $itemid : ''));
											}
											else
											{
												// date in the past
		...
		.find('a.vr-sight-cal-box[data-id]')
			.each(function() {
				jQuery(this).on('click', function(e) {
					...
					var url = '<?php echo JRoute::_('index.php?option=com_vikrestaurants&task=opreservation.edit&from=opdashboard' . ($itemid ? '&Itemid=' . $itemid : ''), false); ?>';
