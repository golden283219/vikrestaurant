...
(function($) {
	...
	var vikPopupMenuInit = function(root, options) {
		...
		$(document).on('keydown.contextmenu.vikrestaurants', function(event) {
			// ignore the event with this namespace because it will end up
			// to catch also the plain keydown event
			if (event.namespace == 'contextmenu.vikrestaurants') {
				return true;
			}
	...
	var vikPopupMenuShow = function(root, event) {
			...
			if (btn.shortcut) {
				...
				var cmd = btn.shortcut.map(function(k) {
					...
					switch (k) {
						...
						// arrow up
						case 37:      k = '<i class="fas fa-arrow-left"></i>'; break;
						// arrow up
						case 38:      k = '<i class="fas fa-arrow-up"></i>'; break;
						// arrow right
						case 39:      k = '<i class="fas fa-arrow-right"></i>'; break;
						// arrow down
						case 40:      k = '<i class="fas fa-arrow-down"></i>'; break;
						// character
		...
		$(document).on('mousedown.contextmenu.vikrestaurants', function(event) {
			// ignore the event with this namespace because it will end up
	...
	var vikPopupMenuHide = function(root) {
		...
		if ($('.vik-context-menu').length) {
			...
			// turn off proxy
			$(document).off('mousedown.contextmenu.vikrestaurants');
	...
	var vikPopupMenuDestroy = function(root) {
		...
		$(document).off('keydown.contextmenu.vikrestaurants');
	...
	$(document).on('mousedown', function() {
		...
		$(document).trigger('mousedown.contextmenu.vikrestaurants');
	});
	...
	$(document).on('keydown', function() {
		...
		$(document).trigger('keydown.contextmenu.vikrestaurants');
	});
