 ...
 * @since 	1.6
 */
class VREInvoiceTakeaway extends VREInvoice
{
	/**
	 * @override
	...
	protected function getPageTemplate()
	{
		...
		return JLayoutHelper::render('templates.invoice.takeaway', $data, $base);
	}
	...
	protected function parseTemplate($tmpl)
	{
		...
		$net         = $this->order->total_to_pay - $this->order->taxes - $this->order->pay_charge - $this->order->delivery_charge - $this->order->tip_amount;
		$discount    = $this->order->discount_val;
		$pay_ch      = $this->order->pay_charge;
		$delivery_ch = $this->order->delivery_charge;
		$taxes       = $this->order->taxes;
		$gratuity    = $this->order->tip_amount;
		$grand_total = $this->order->total_to_pay;
		...
		$tmpl = str_replace('{invoice_paycharge}'     , $currency->format($pay_ch)         , $tmpl);
		$tmpl = str_replace('{invoice_deliverycharge}', $currency->format($delivery_ch)    , $tmpl);
