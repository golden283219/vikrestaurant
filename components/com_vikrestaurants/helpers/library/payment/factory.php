...
final class VREPaymentFactory
{
	...
	public static function getSupportedDrivers()
	{
		 ...
		 */
		$files = glob(VREADMIN . DIRECTORY_SEPARATOR . 'payments' . DIRECTORY_SEPARATOR . '*.php');
		 ...
		 * @since 	1.8.5
		 */
		$results = $dispatcher->trigger('onLoadSupportedPaymentMethods', array('vikrestaurants'));
	...
	public static function getPaymentConfig($payment)
	{
		...
		else
		{
			 ...
			 * @since 	1.8.5
			 */
			$config = $dispatcher->triggerOnce('onLoadPaymentMethodConfigurationForm', array('vikrestaurants', $payment));
	...
	public static function getPaymentInstance($payment, $order = array(), $config = array())
	{
		...
		else
		{
			 ...
			 */
			VRELoader::import('library.payment.plugin');
			// the payment process will be handled by a third-party plugin
			$processor = new VREPaymentPlugin($payment, $order, $config);
		}
