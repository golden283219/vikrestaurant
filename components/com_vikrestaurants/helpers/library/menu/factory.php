...
VRELoader::import('library.menu.abstractlist');
...
abstract class MenuFactory
{
	...
	public static function getMenuType()
	{
		...
		if (is_null(static::$menuType))
		{
			 ...
			 * @since 	1.8
			 */
			static::$menuType = VREFactory::getEventDispatcher()->triggerOnce('onBeforeDefineVikRestaurantsMenuType');
	...
	protected static function create($alias, $parent, array $args = array())
	{
		if (!isset(static::$classMap[$alias]))
		{
			...
			if (!VRELoader::import('library.menu.' . $type . '.' . $alias))
			{
				// menu type not found
