...
abstract class VREApplication
{
	...
	public static function getInstance()
	{
		if (static::$instance === null)
		{
			...
			if (!VRELoader::import('library.adapter.platform.' . $platform))
			{
				throw new Exception(sprintf('Application [%s] not found', $platform), 404);
			}

			// define class name
			$class = 'VREApplication' . ucfirst($platform);

			// check if the class exists and inherits VREApplication abstraction
			if (class_exists($class) && is_subclass_of($class, 'VREApplication'))
			{
	...
	public function radioYesNo($name, $elem_1, $elem_2, $wrapped = true, $layout = null)
	{
		...
		if (!$layout)
		{
			...
			$layout = VREFactory::getConfig()->get('uiradio', 'ios');
		}
		
		// load radio widget
		VRELoader::import('library.widget.radio.' . $layout);

		// build radio classname
		$radio_class = 'VRERadio' . ucwords($layout);

		// instantiate radio class
	 ...
	 * @since 	1.6
	 */
	abstract public function attachPopover($selector = '.vrPopover', array $options = array());
...
JS
	...
	public function getThemeClass($suffix = null)
	{
		...
		$theme = VREFactory::getConfig()->get('sitetheme', '');
		$class = array();
	 ...
	 * @since 	1.8
	 */
	abstract public function onContentPrepare(&$content, $params = array());
	 ...
	 * @since 	1.8.2
	 */
	abstract public function getOperatorUsers($id = 0);
