 ...
 *
 * @since 1.7.4
 * @since 1.8.2 Renamed from UIEventDispatcher
 */
class VREEventDispatcher
{
	/**
	 * A list of instances.
	 *
	...
	public function __construct(array $options = array(), $dispatcher = null)
	{
		...
		// always import plugins that belong to "vikrestaurants" and "e4j" folders
		$this->import(array('vikrestaurants', 'e4j'));
	}
	...
	/**
	 * Triggers an event by dispatching arguments to all observers that handle
	 * the specified event and returning only the first successful value.
	 *
	 * @param 	mixed 	$event  The event to trigger or an array containing the 
	 * 							event name and the plugins folder to import.
	 * @param 	array 	$args   An array of arguments.
	 *
	 * @return 	mixed 	The first successful value.
	 *
	 * @uses 	trigger()
	 */
