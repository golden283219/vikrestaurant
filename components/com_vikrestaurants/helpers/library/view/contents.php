...
/**
 * VikRestaurants view contents handler.
 * This class provides helpful tools to enhance the
 * <head> of the pages.
 *
 * @since 1.8
 */
class VREViewContents
{
	/**
	 * A list of instances.
	 *
	...
	{
		if (!preg_match("/VikRestaurantsView(.+)$/i", $name, $match))
		{
			// the view is not part of VikRestaurants
			return null;
		}

		$file 	= strtolower($match[1]);
		$class 	= 'VREViewContents' . ucwords($match[1]);

		if (!VRELoader::import('library.view.classes.' . $file))
		{
		...
		if (!$handler instanceof VREViewContents)
		{
	...
	{
		/**
		 * Include a flag to understand where VikRestaurants body starts.
		 *
		 * @since 1.8.1
		 */
		$html = JLayoutHelper::render('document.start');
	...
	public function buildOpenGraph()
	{
		...
		foreach (VikRestaurants::getKnownLanguages() as $tag)
		{
		 ...
		 * @param 	VREViewContents	 $this  The view content handler. 
		 *
		 * @return 	void
		 *
		 * @since 	1.8
		 */
		VREFactory::getEventDispatcher()->trigger('onBuildOpenGraph', array($doc, $this));
	...
	public function buildMicrodata()
	{
		 ...
		 * @param 	boolean 	 	 &$res   True whether the JSON object should be included to the document.
		 * @param 	VREViewContents	 $this   The view content handler. 
		 *
		 * @return 	void
		 *
		 * @since 	1.8
		 */
		VREFactory::getEventDispatcher()->trigger('onBeforeAddMicrodata', array(&$json, &$res, $this));
